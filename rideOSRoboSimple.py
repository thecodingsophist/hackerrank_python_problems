...
class RobotRouter:
    ...
    def moveX(self, pointA, pointB):
        next = pointA
        #change the x coordinate according to direction
        if self.shortestDistanceX(pointA, pointB) == "W":
            next.x = next.x - 1
        if self.shortestDistanceX(pointA, pointB) == "E":
            next.x = next.x + 1
        if self.shortestDistanceX(pointA, pointB) == None:
            next.x = next.x
        return next

    def moveY(self, pointA, pointB):
        #change the y coordinate according to direction
        next = pointA
        if self.shortestDistanceY(pointA, pointB) == "S":
            next.y = next.y - 1
        if self.shortestDistanceY(pointA, pointB) == "N":
            next.y = next.y + 1
        if self.shortestDistanceY(pointA, pointB) == None:
            next.y = next.y
        return next

    def shortestDistanceX(self, pointA, pointB):
    ...
    def shortestDistanceY(self, pointA, pointB):
    ...
    def route(self, origin, destination):
        ...
        pointA = origin
        pointB = destination
        while pointA != destination:
            if pointA.x != pointB.x:
                next = self.moveX(pointA, pointB)
                nextPoint = Coordinate(next.x, next.y)
                # print("nextX:", nextPoint)
                list.append(nextPoint)
                # print("list:", str(list))
            elif pointA.y != pointB.y:
                next = self.moveY(pointA, pointB)
                nextPoint = Coordinate(next.x, next.y)
                # print("nextY:", nextPoint)
                list.append(nextPoint)
                # print("list:", list)
        return list
